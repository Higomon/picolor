...
一般的な測色器へ入れにくい試料の色変化も、その場で記録できる。
...
測定中は、[**GIN-ICHI（銀一）シルクグレーカードVer.2**](https://www.ginichi.com/shop/g/g13819/)の18%グレー面を`Ref`枠へ入れ、相対基準として使う。

## 再現性を保つ要点

> [!IMPORTANT]
> **色測定で最も重要なのは、カメラ性能より照明と色基準の管理である。**

| 管理対象 | 要点 |
|---|---|
| 照明 | 位置、角度、距離、明るさ、色味、拡散条件の固定 |
| 周囲光 | [HAKUBA LEDスタジオボックス60](https://www.amazon.co.jp/dp/B0923V3439)などの簡易暗室で遮光、または同一型番の[撮影用LED照明（Ulanziなど）](https://www.ulanzi.jp/collections/led-rgb-video-lights)を3セット配置 |
| 軽い照明劣化 | 明るさだけの軽い低下であれば、同じ色基準を同じ条件で使うことで相対的な再現性を維持しやすい |
| 再校正 | 照明の色味、むら、角度、距離が変化した場合は、18%グレーカードだけでは補正しきれないためSpyder Checkrで再校正 |
| 色基準 | 同じ製品を、同じ面、向き、位置で使用 |

簡易暗室または3セットの固定照明により、室内灯や窓からの光が変化しても試料へ届く光を制御しやすくなり、測定の再現性を保ちやすくなる。

> [!CAUTION]
> **Spyder Checkrとグレーカードの色面を素手で触らないこと。** 皮脂、汚れ、傷、退色は基準の色を変え、測定の再現性を低下させる。端を持つか清潔な手袋を使い、使用後は遮光・防塵して保管する。汚れや退色がある場合は交換する。
...
| 色基準1 | [色基準チャート（Spyder Checkr 48色モデル）](https://www.datacolor.jp/camera-solution/spyder-checkr.html) | 48色の既知Lab値による色補正に使用 |
...
- 同一画像内の`Ref`／`Target`同時測定による照明変動の影響低減
- 既知Lab値を持つ色基準（Spyder Checkr）を絶対基準、18%グレーカードを相対基準とする運用
- 暗所ノイズ、照明むら、白バランスの補正
- 波長別スペクトルおよびUV-Vis吸光度の測定には非対応
- 光沢、泡、影、周囲光、距離、角度の変化による測定値への影響
- カメラ、照明、距離、絞り、基準配置の変更時に必要な再校正
- 市販測色計と同等の精度保証は対象外
- 用途ごとの既知試料による検証
